...
if (isset($_POST['gerar_pdf'])) {
    ...
    try {
        ...
        $result = $DB->query("SHOW TABLES LIKE 'glpi_termos_cabecalho'");
        if (!$result || $DB->numrows($result) == 0) {
            Session::addMessageAfterRedirect('Tabela de cabeçalhos não encontrada. Verifique se o plugin foi instalado corretamente.', true, ERROR);
            Html::redirect($_SERVER['REQUEST_URI']);
            exit;
        }
...
<table class="header-table">
    <tr>
        ...
        $cpf_display = !empty($cpf) ? htmlspecialchars($cpf) : '_______________';
        $cargo_display = !empty($cargo) ? htmlspecialchars($cargo) : '_________________________________';
        ...
        // SEÇÃO ATUALIZADA: Tabela com equipamentos do usuário (computadores, monitores, telefones, linhas e rádios)
        if (!empty($all_equipment)) {
            ...
            foreach ($all_equipment as $equipment) {
                // Concatenar informações baseado no tipo de equipamento
                $descricao_partes = [];
                
                if ($equipment['tipo'] === 'Linha Telefônica') {
                    // Para linhas telefônicas, usar os campos específicos
                    if (!empty($equipment['prefixo'])) {
                        $descricao_partes[] = 'Prefixo: ' . $equipment['prefixo'];
                    }
                    
                    if (!empty($equipment['operadora'])) {
                        $descricao_partes[] = 'Operadora: ' . $equipment['operadora'];
                    }
                    
                    if (!empty($equipment['tipo_linha'])) {
                        $descricao_partes[] = 'Tipo: ' . $equipment['tipo_linha'];
                    } elseif (!empty($equipment['linetype'])) {
                        $descricao_partes[] = 'Tipo: ' . $equipment['linetype'];
                    }
                    
                    if (!empty($equipment['numero_chamado'])) {
                        $descricao_partes[] = 'Nº: ' . $equipment['numero_chamado'];
                    } elseif (!empty($equipment['caller_num'])) {
                        $descricao_partes[] = 'Nº: ' . $equipment['caller_num'];
                    }
                    
                    if (!empty($equipment['nome_linha'])) {
                        $descricao_partes[] = 'Nome: ' . $equipment['nome_linha'];
                    } elseif (!empty($equipment['caller_name'])) {
                        $descricao_partes[] = 'Nome: ' . $equipment['caller_name'];
                    }
                    
                    if (!empty($equipment['serial_equip'])) {
                        $descricao_partes[] = 'Serial Equip: ' . $equipment['serial_equip'];
                    }
                    ...
                    // Para outros equipamentos (computador, monitor, telefone)
                    ...
                    if ($equipment['tipo'] === 'Telefone' && !empty($equipment['number_line'])) {
                        $descricao_partes[] = 'Tel: ' . $equipment['number_line'];
                    }
                    
                    // Adicionar marca do telefone se não tiver fabricante
                    if ($equipment['tipo'] === 'Telefone' && empty($equipment['fabricante']) && !empty($equipment['brand'])) {
